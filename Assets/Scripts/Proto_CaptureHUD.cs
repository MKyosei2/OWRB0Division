...
{
    /// <summary>
    /// 録画向け：タイムコード/Take/簡易FPS/撮影チェックを表示。
    /// DebugHUDとは別に「提出向けの情報」だけ持つ。
    /// </summary>
    public class Proto_CaptureHUD : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (Input.GetKeyDown(takeUpKey)) { takeNumber++; SubtitleManager.Instance?.Add($"【TAKE】{takeNumber}", 1.0f); }
            if (Input.GetKeyDown(takeDownKey)) { takeNumber = Mathf.Max(1, takeNumber - 1); SubtitleManager.Instance?.Add($"【TAKE】{takeNumber}", 1.0f); }
        ...
        private static string FormatTimecode(float seconds)
        {
            ...
            int ff = Mathf.FloorToInt((seconds - total) * 30f); // 30fps想定の簡易
